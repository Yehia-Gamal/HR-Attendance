...
create table if not exists public.attendance_events (
  ...
  biometric_method text default 'selfie',
...
begin
  ...
  insert into public.profiles (
    id, email, full_name, avatar_url, employee_id, role_id, branch_id, department_id, governorate_id, complex_id,
...
select 'MAIN', 'مجمع منيل شيحة', 'مجمع منيل شيحة - الجيزة', g.id, c.id, 29.95109939158933::numeric, 31.238741920853883::numeric, 300, 2000
...
set latitude = 29.95109939158933::numeric,
    longitude = 31.238741920853883::numeric,
...
select * from (
  select 'EMP-001' employee_code, 'الشيخ محمد يوسف' full_name, '01000000001' phone, 'mohamed.youssef@ahla.local' email, 'المدير التنفيذي' job_title, (select id from public.roles where slug='executive') role_id, (select id from public.branches where code='MAIN') branch_id, (select id from public.departments where code='EXEC') department_id, (select id from public.governorates where code='GZ') governorate_id, (select id from public.complexes where code='AHLA-MANIL') complex_id, (select id from public.shifts where name='وردية 9ص إلى 5م' limit 1) shift_id, 'ACTIVE' status, current_date hire_date
  union all select 'EMP-002','يحيى جمال السبع','01000000002','yahia.secretary@ahla.local','السكرتير التنفيذي',(select id from public.roles where slug='executive-secretary'),(select id from public.branches where code='MAIN'),(select id from public.departments where code='EXEC'),(select id from public.governorates where code='GZ'),(select id from public.complexes where code='AHLA-MANIL'),(select id from public.shifts where name='وردية 9ص إلى 5م' limit 1),'ACTIVE',current_date
